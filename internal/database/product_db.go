...
import (
	...
	"github.com/lleandrroo/ecommerce_fullcycle/internal/entity"
)
...
func (pd *ProductDB) GetProducts() ([]*entity.Product, error) {
	rows, err := pd.db.Query("SELECT id, name, category_id, price FROM products")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (pd *ProductDB) GetProduct(id string) (*entity.Product, error) {
	var product entity.Product
	err := pd.db.QueryRow("SELECT id, name, category_id, image_url, price  FROM products WHERE id = ?", id).Scan(&product.ID, &product.Name, &product.ImageURL)
	if err != nil {
		return nil, err
	}
	return &product, nil
...
func (pd *ProductDB) GetProductByCategoryID(categoryID string) ([]*entity.Product, error) {
	...
	for rows.Next() {
		var product entity.Product
		if err := rows.Scan(&product.ID, &product.Name, &product.CategoryID, product.ImageURL, &product.Price); err != nil {
			return nil, err
		}
		products = append(products, &product)
	}
